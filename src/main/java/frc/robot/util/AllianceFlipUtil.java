...
public class AllianceFlipUtil {
  ...
  public static Rotation3d apply(Rotation3d rotation) {
    if (shouldFlip()) {
        //need to project into XZ plane then invert x component
        return new Rotation3d(rotation.getX(), apply(new Rotation2d(rotation.getY())).getRadians(), rotation.getZ());
    } else {
        return rotation;
    }
  }
  ...
  public static Pose3d apply(Pose3d pose3d) {
    if (shouldFlip()) {
        return new Pose3d( 
            apply(pose3d.getTranslation()), apply(pose3d.getRotation()));
    } else {
        return pose3d;
    }
  }
