...
public class ConnorLineup extends Command {
   ...
   //private Translation2d rotateNotClockwise = new Translation2d(rotateX , -rotateY); // 30 degrees counterclockwise (or anticlockwise)
    ...
    double rotateClockwise = 0.1;
    double rotateNotClockwise = -0.1;

    private double OFFSET_THRESHOLD = 0.01;
    private double vertOFFSET_THRESHOLD = 0.01;
   private double angOFFSET_THRESHOLD = 0.1;
    ...
    private double kTargetDistance = 1.2192;
     ...
     private double kLimelightHeight = 0.597;
    ...
    public void execute() {
       ...
       if (limelightTable.getEntry("tv").getDouble(0) != 0) {
            ...
            while (Math.abs(idealHeading - heading) > angOFFSET_THRESHOLD){
                ...
                drive.drive(noMove,rotate,false);
            ...
            kDistance = kLimelightDiff/Math.atan(kLimeLightAngle + kLimeLightVerticalAngle);
